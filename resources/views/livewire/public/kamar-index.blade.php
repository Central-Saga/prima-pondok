...
<section class="py-12 sm:py-16 bg-white">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl sm:text-3xl font-semibold text-slate-900">Semua Kamar</h1>
                <p class="mt-1 text-slate-600">Pilih kamar yang tersedia lalu lanjut ke detail untuk memesan.</p>
            </div>
            <div class="flex items-center gap-3">
                <input type="text" wire:model.live.debounce.800ms="q" class="ui-input w-64" placeholder="Cari kamar atau tipe..." />
                <a href="{{ route('home') }}" class="ui-btn-secondary">Kembali</a>
            </div>
        </div>
        ...
        <div class="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($this->items as $item)
                <div class="rounded-xl border border-sky-100 bg-white shadow-sm overflow-hidden transform-gpu transition duration-300 ease-out hover:scale-[1.02] hover:shadow-md hover:border-sky-200">
                    ...
                    @else
                        <div class="h-44 bg-slate-100 flex items-center justify-center text-slate-400">Foto</div>
                    @endif
                    <div class="p-4">
                        ...
                        <div class="mt-4">
                            <a href="{{ route('kamar.show', $item->id) }}" class="ui-btn-primary">Lihat Detail</a>
                        </div>
                    </div>
                </div>
            @empty
                <div class="col-span-full text-slate-600">Belum ada kamar tersedia.</div>
            @endforelse
        </div>
