...
new class extends Component {
    ...
    public function save(): void
    {
        ...
        if (!empty($images)) {
            $existingFotos = $this->kamar->fotos ?? [];
            ...
            $this->kamar->update(['fotos' => $existingFotos]);
            $this->images = [];
        }
        $this->redirectRoute('admin.kamar.index');
    }

    public function deleteFoto(int $index): void
    {
        $fotos = $this->kamar->fotos ?? [];
        if (isset($fotos[$index])) {
            $path = $fotos[$index];
            Storage::disk('public')->delete($path);
            unset($fotos[$index]);
            $this->kamar->update(['fotos' => array_values($fotos)]);
        }
        $this->kamar->refresh();
    }
    ...
    <form wire:submit="save" class="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4 ui-card">
        ...
        <div class="sm:col-span-2">
            ...
            <div class="mt-3 grid grid-cols-3 sm:grid-cols-4 gap-2">
                @php $currentFotos = $kamar->fotos ?? []; @endphp
                @forelse($currentFotos as $idx => $path)
                    <div class="relative group">
                        <div class="aspect-[4/3] overflow-hidden rounded border bg-slate-100">
                            <img src="{{ asset('storage/'.ltrim($path,'/')) }}" class="h-full w-full object-cover" />
                        </div>
                        <button type="button" wire:click="deleteFoto({{ $idx }})" class="absolute top-1 right-1 hidden group-hover:inline-flex items-center rounded-md px-2 py-1 text-xs font-medium text-rose-700 ring-1 ring-inset ring-rose-200 bg-white/90 hover:bg-rose-50">Hapus</button>
                    </div>
                @empty
